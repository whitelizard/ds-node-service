      ...
      contentProps: {
        ...
        markerWidths: [
          ...
          [
            'fn',
            ['step'],
            ['if', ['=', ['%', 'step', 50], 1], 1.5, ['if', ['=', ['%', 'step', 10], 0], 1.5, 2.5]],
          ],
        ],
        markerLength: 12,
        markerColors: [
...
test('Try joi 1', async t => {
  try {
    const result = joi.attempt(assetType1, assetType, 'error message');
    // console.log(result);
...
test('Try joi 2', async t => {
  const result = joi.validate(assetType1, assetType);
